<template>
  <div class="CodesErstellen">
    <Header 
        ...
        :task_name_code="'TopSortErkennen_Backen'"
        :task_number="'13'" 
...
*/
    ...
    path_exists(u,v){
      ...
      for(let j = 0; j < neighbours.length; j++){
        if(this.nodes[neighbours[j]].active){   //uns interessieren nur die aktiven, nicht die anderen
          Q.push(neighbours[j])
        }
      }
      while(!Q.length <= 0){
        ...
        if(node == v){
          ...
          var neighbours = this.adj_list[this.nodes[node].id]
          for(let j = 0; j < neighbours.length; j++){
            if(this.nodes[neighbours[j]].active){   //uns interessieren nur die aktiven, nicht die anderen
              Q.push(neighbours[j])
            }
